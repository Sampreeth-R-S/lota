...
class Linear(nn.Linear, VeraLayer):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        if self.enable_quantization:
            # Quantization configurations for 4-bit quantization
            self.quant_config = quantization.QConfig(
                activation=quantization.FakeQuantize.with_args(
                    observer=quantization.MinMaxObserver, quant_min=0, quant_max=15, dtype=torch.qint8
                ),
    ...
    def prepare_for_quantization(self):
        """Prepare the layer for quantization-aware training."""
        if not self.enable_quantization:
            return
        
        # Apply quantization-aware training
        self.qconfig = self.quant_config
        quantization.prepare_qat(self, inplace=True)

    def quantize_weights(self):
        """Quantize the weights to 4-bit representation."""
        if not self.enable_quantization:
            return
    ...
    def forward(self, x: torch.Tensor, *args, **kwargs) -> torch.Tensor:
        if self.enable_quantization:
            # Quantize the weights before the forward pass
